...
# Remove unnecessary attachment, keep only Video, Audio, English Subtitle
ffmpeg -i "$1" -map 0:0 -map 0:1 -map 0:2 -c:v copy -c:a copy -c:s:2 copy -metadata:s:a:0 language=jpn "$1_trim.mkv" -y

# Extract fonts, install and update cache
ffmpeg -dump_attachment:t "" -i "$1_trim.mkv" -y
...
/usr/bin/ffmpeg -i "$1_trim.mkv" -map 0:s "$subtitle" -y

# Stage 2:	demux $1(mkv), remove original subtitle
/usr/bin/ffmpeg -i "$1_trim.mkv" -map 0 -map 0:s -codec copy "$1_tmp.mkv" -y
